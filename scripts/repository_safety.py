...
def classify(path: str, data: bytes) -> set[str]:
    ...
    if PurePosixPath(path.lower()).suffix in {
        ".json",
        ".jsonl",
        ".csv",
        ".tsv",
    ...
    if PurePosixPath(path.lower()).suffix in {".csv", ".tsv"}:
        ...
        if body.strip() and re.search(
            rb"(?i)(patient_name|patient_dob|member_id|insured_id_number|ssn)", header
        ):
            reasons.add("POSSIBLE_PHI")
    return reasons
...
def scan(
    history: bool = False, removal_paths: set[str] | None = None, ref: str | None = None
) -> dict:
    revision = (
        git("rev-parse", "--verify", "--end-of-options", ref + "^{commit}").decode().strip()
        if ref
        else "--all"
    )
    if history:
        ...
        for entry in git("ls-files", "--stage", "-z").split(b"\x00"):
            if entry:
                ...
                candidates.append([metadata.split()[1], staged_path])
    findings = []
    if history:
        # rev-list assigns one name to reused blobs; inspect every historical name too.
        names = git("log", revision, "--format=", "--name-only", "-z", "--no-renames").split(
            b"\x00"
        )
    ...
    return {
        ...
        "scope": ("SELECTED_ANCESTRY" if ref else "ALL_LOCAL_REFS") if history else "INDEX",
...
def main() -> int:
    ...
    parser.add_argument(
        "--ref", help="Restrict history audit to this commit ancestry; default audits all refs"
    )
