...
from packages.document_routing.visual.features import (
...
def run(train_source,test_source):
 ...
 meta={"model_id":out.name,"model_type":"HOG_LOGISTIC","model_version":"1.0.0","feature_version":VISUAL_FEATURE_VERSION,"training_source":train_source,"classes":list(model.classes_),"model_file":path.name,"model_sha256":hashlib.sha256(path.read_bytes()).hexdigest(),"model_size_bytes":path.stat().st_size,"git_sha":subprocess.run(check=True, args=["git","rev-parse","HEAD"],cwd=ROOT,text=True,capture_output=True).stdout.strip(),"created_at":datetime.now(UTC).isoformat(),"status":"EXPERIMENTAL"};(out/"metadata.json").write_text(json.dumps(meta,indent=2),"utf-8")
 probs=[];lat=[]
 for r in test:
