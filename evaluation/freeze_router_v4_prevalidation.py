...
def freeze():
    git=lambda *a: subprocess.run(check=True, args=["git",*a],cwd=ROOT,text=True,capture_output=True).stdout.strip()
    status=git("status","--porcelain","--untracked-files=no")
    value={"freeze_id":"ROUTER_V4_PREVALIDATION_BASELINE","created_at":datetime.now(UTC).isoformat(),
      ...
      "decision_contract_version":"4.0","ocr":{"engine":"tesseract-cli","version":subprocess.run(check=True, args=
