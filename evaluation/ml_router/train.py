...
def train(model_type,source):
 ...
 if model_type=="lightgbm":
  ...
  model=lgb.LGBMClassifier(objective="multiclass",num_class=len(CLASSES),learning_rate=.05,num_leaves=15,verbosity=-1,**params);model.fit(x,y,feature_name=FEATURE_NAMES);importance=dict(zip(FEATURE_NAMES,map(float,model.feature_importances_)));ext="txt"
 else:
  import xgboost as xgb
  model=xgb.XGBClassifier(objective="multi:softprob",num_class=len(CLASSES),learning_rate=.05,verbosity=0,**params);model.fit(x,y);importance=dict(zip(FEATURE_NAMES,map(float,model.feature_importances_)));ext="json"
