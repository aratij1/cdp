...
def test_multiline_addresses_use_region_mode_and_canonical_evidence_is_preserved():
    ...
    p=expanded["patient_name"][0];assert ("line",(p.x0,p.y0,p.x1,p.y1)) in calls
    ...
    assert provenance.ocr_crop_bbox.x0==p.x0
...
def test_rendering_npi_valid_rows_only(values,expected,ambiguous):
    ...
    assert f.raw_value==expected and len(f.candidates)==6
