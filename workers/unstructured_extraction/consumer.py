...
class UnstructuredExtractionWorker:
    ...
    async def handle_one(self, envelope: EventEnvelope) -> None:
        ...
        if processing_route is not None:
            ...
            if target not in {
                ExtractionTarget.UNKNOWN_STRUCTURED_LAYOUT,
                ExtractionTarget.UNKNOWN_UNSTRUCTURED_LAYOUT,
            }:
                raise ValueError(
                    f"PROCESSING_ROUTE_TARGET_MISMATCH:{processing_route}:{target.value}"
                )
        ...
        with self._session_factory() as session:
            ...
            images = {
                p.page_number: await asyncio.to_thread(
                    _load_image, self._object_store, p.extraction_object
                )
                for p in pages
            }
            page_lines = {
                number: await asyncio.to_thread(self._text_extractor.extract, image)
            ...
            if family.family and family.page_number and not force_layout_graph:
                ...
                for field_name, crop in crops.items():
                    ...
                    value = " ".join(
                        line.text for line in sorted(lines, key=lambda x: (x.y0, x.x0))
                    )
                    confidence = (
                    ...
                    extracted.append(
                        ExtractedField(
                            ...
                            confidence=confidence,
                            page_number=family.page_number,
                            bounding_box=box,
                            ...
                            validation_reasons=[]
                            if valid and value
                            else ["UNSTRUCTURED_FIELD_UNRESOLVED"],
                            candidates=[evidence],
                        )
                    )
            layout_results = []
                ...
                for page_number, image in images.items():
                    ...
                    for field_name, candidates in result.candidates.items():
                        ...
                        field_type = (
                            "currency"
                            if any(
                                c in field_name.lower()
                                for c in ["charge", "paid", "amount", "due", "balance", "total"]
                            )
                            else (
                                "date"
                        ...
                        ocr_candidates = [
                            OCRCandidate(
                                ...
                                model_name=getattr(
                                    self._text_extractor, "model_name", result.engine
                                ),
                                ...
                                validation_results=(
                                    "DATATYPE_VALID",
                                    item.relationship_evidence.relationship,
                                )
                                if bool(item.datatype_valid or normalize(field_type, item.value)[1])
                                else (item.relationship_evidence.relationship,),
                        ...
                        decision = self._decisions.decide(
                            DecisionContext(
                                ...
                                deterministic_evidence={"DATATYPE_VALID"}
                                if effective_datatype_valid
                                else set(),
                                ...
                                validation_reasons=list(
                                    dict.fromkeys(
                                        [
                                            ...
                                            f"LABEL_ALIAS:{best.matched_alias}",
                ...
                OutboxRecord(
                    ...
                    envelope=EventEnvelope(
                        ...
                        "claim_membership": envelope.payload.get("claim_membership") or {},
                            ...
                            or (
                                layout_results[0].schema_evidence.schema_family
                                if layout_results
                                else "unknown_unstructured"
                            ),
                            "family_confidence": family.score
                            or (
                                layout_results[0].schema_evidence.confidence
                                if layout_results
                                else 0.0
                            ),
                            "generic_route": layout_results[0].route.value
                            if layout_results
                            else None,
                            "route_reason_codes": layout_results[0].route_reason_codes
                            if layout_results
                            else [],
                            "reason_codes": []
                            if extracted
                            else ["UNSTRUCTURED_SCHEMA_OR_ANCHOR_UNRESOLVED"],
                        },
                    ),
